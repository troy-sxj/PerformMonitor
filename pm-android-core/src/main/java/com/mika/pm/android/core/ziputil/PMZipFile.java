...
/**
 * @Author: mika
 * @Time: 2019-11-05 18:05
 * @Description:
 */
public class PMZipFile implements Closeable, ZipConstants {
    /**
    ...
    private final LinkedHashMap<String, PMZipEntry> entries = new LinkedHashMap<String, PMZipEntry>();
     ...
     */
    public PMZipFile(File file) throws ZipException, IOException {
        this(file, OPEN_READ);
    }
     ...
     */
    public PMZipFile(String name) throws IOException {
     ...
     * @throws IOException if an {@code IOException} occurs.
     */
    public PMZipFile(File file, int mode) throws IOException {
     ...
     * need random-access entry lookup by name, you should probably use {@link InputStream}
     * instead, to avoid paying to construct the in-memory index.
     *
     * @throws IllegalStateException if this zip file has been closed.
     */
    public Enumeration<? extends PMZipEntry> entries() {
        checkNotClosed();
        final Iterator<PMZipEntry> iterator = entries.values().iterator();
        return new Enumeration<PMZipEntry>() {
            public boolean hasMoreElements() {
                checkNotClosed();
                return iterator.hasNext();
            }

            public PMZipEntry nextElement() {
                checkNotClosed();
     ...
     * See {@link java.util.zip.ZipOutputStream#setComment}.
     *
     * @throws IllegalStateException if this zip file has been closed.
     * @since 1.7
     */
     ...
     * @throws IllegalStateException if this zip file has been closed.
     */
    public PMZipEntry getEntry(String entryName) {
        checkNotClosed();
        if (entryName == null) {
            throw new NullPointerException("entryName == null");
        }
        PMZipEntry ze = entries.get(entryName);
        if (ze == null) {
     ...
     * @throws IllegalStateException if this zip file has been closed.
     */
    public InputStream getInputStream(PMZipEntry entry) throws IOException {
        ...
        synchronized (localRaf) {
            ...
            if ((gpbf & PMZipFile.GPBF_UNSUPPORTED_MASK) != 0) {
            ...
            if (entry.compressionMethod == PMZipEntry.STORED) {
                rafStream.endOffset = rafStream.offset + entry.size;
            } else {
                rafStream.endOffset = rafStream.offset + entry.compressedSize;
            }
    ...
    private void readCentralDir() throws IOException {
        ...
        for (int i = 0; i < numEntries; ++i) {
            PMZipEntry newEntry = new PMZipEntry(hdrBuf, bufferedStream, StandardCharsets.UTF_8,
                    (false) /* isZip64 */);
