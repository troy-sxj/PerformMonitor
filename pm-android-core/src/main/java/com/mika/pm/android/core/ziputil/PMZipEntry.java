 ...
 * @Description:
 */
public class PMZipEntry implements ZipConstants, Cloneable {
    /**
    ...
    public PMZipEntry(String name, String comment, long crc, long compressedSize,
     ...
     */
    public PMZipEntry(PMZipEntry ze) {
        name = ze.name;
        comment = ze.comment;
        time = ze.time;
        size = ze.size;
    ...
    public PMZipEntry(PMZipEntry ze, String name) {
        this.name = name;
        comment = ze.comment;
        time = ze.time;
        size = ze.size;
     ...
     */
    PMZipEntry(byte[] cdeHdrBuf, InputStream cdStream, Charset defaultCharset, boolean isZip64) throws IOException {
        ...
        int sig = it.readInt();
        if (sig != CENSIG) {
            PMZipFile.throwZipException("unknown", cdStream.available(), "unknown", 0, "Central Directory Entry", sig);
        }
        it.seek(8);
        ...
        if ((gpbf & PMZipFile.GPBF_UTF8_FLAG) != 0) {
    ...
    @Override public Object clone() {
        try {
            PMZipEntry result = (PMZipEntry) super.clone();
